...
function AdminUsers() {
  ...
  const load = async () => {
    ...
    const { data } = await supabase
      .from("profiles")
      .select(
        "id,first_name,last_name,email,contact,country,currency,balance,total_invested,total_withdrawn,is_blocked,profile_locked,created_at",
      )
  ...
  const revoke = async (u: Row, role: string) => {
    if (!isSuperAdmin) return toast.error("Only super-admin can change roles");
    if (role === "super_admin" && u.id === user?.id)
      return toast.error("You cannot revoke your own super-admin");
    setBusy(u.id);
    const { error } = await supabase
      .from("user_roles")
      .delete()
      .eq("user_id", u.id)
      .eq("role", role as "admin" | "super_admin" | "user");
    setBusy(null);
    if (error) return toast.error(error.message);
          ...
          return (
            <article key={u.id} className="rounded-2xl border border-border bg-card p-4">
              <header className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-1.5">
                    ...
                    {isSuper && (
                      <Badge tone="warning" icon={<Crown className="h-2.5 w-2.5" />}>
                        Super
                      </Badge>
                    )}
                    {isAdmin && !isSuper && (
                      <Badge tone="primary" icon={<ShieldCheck className="h-2.5 w-2.5" />}>
                        Admin
                      </Badge>
                    )}
                ...
                <div className="text-right">
                  <div className="text-sm font-bold">
                    {formatMoney(Number(u.balance), u.currency)}
                  </div>
              ...
              <div className="mt-3 grid grid-cols-2 gap-2 text-[10px] text-muted-foreground sm:grid-cols-3">
                ...
                <Stat
                  label="Withdrawn"
                  value={formatMoney(Number(u.total_withdrawn), u.currency)}
                />
              ...
              <div className="mt-3 flex flex-wrap gap-1.5">
                ...
                <button
                  disabled={busy === u.id}
                  onClick={() => setBlocked(u, !u.is_blocked)}
                ...
                <button
                  disabled={busy === u.id}
                  onClick={() => setProfileLocked(u, !u.profile_locked)}
        ...
        >
          ...
          >
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {creditDialog.first_name} {creditDialog.last_name} · current{" "}
                  {formatMoney(Number(creditDialog.balance), creditDialog.currency)}
                </p>
              </div>
              <button
                onClick={() => setCreditDialog(null)}
            ...
            <p className="mt-2 text-[11px] text-muted-foreground">
              Logged as a completed transaction. Use a negative number to debit.
            </p>
...
function Badge({
  ...
  return (
    <span
      className={`inline-flex items-center gap-0.5 rounded-full border px-1.5 py-0.5 text-[9px] font-semibold uppercase ${map[tone]}`}
    >
      {icon}
      {children}
    </span>
