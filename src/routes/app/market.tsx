...
function MarketTab() {
  ...
  const load = async () => {
    ...
    const [{ data: prods }, { data: invs }] = await Promise.all([
      ...
      supabase
        .from("investments")
        .select(
          "id,product_id,purchase_price,daily_earning,duration_days,total_return,earnings_paid_count,earnings_accrued,status,end_date,products(name)",
        )
  ...
  const confirmBuy = async () => {
    ...
    if (invErr) {
      setBusy(false);
      return toast.error(invErr.message);
    }
    ...
    await supabase
      .from("profiles")
      .update({ balance: profile.balance - localPrice })
      .eq("id", profile.id);
  ...
  return (
    <div className="px-5 pt-6 pb-6">
      ...
      <p className="mt-1 text-xs text-muted-foreground">
        Tap a product for details. Daily earnings credit automatically.
      </p>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <div className="text-sm font-bold text-success">
                        +{formatMoney(inv.daily_earning, cur)}/day
                      </div>
                      <div className="text-[10px] text-muted-foreground">
                        earned {formatMoney(inv.earnings_accrued, cur)}
                      </div>
                    </div>
                  </div>
              ...
              >
                ...
                <div className="mt-4 grid grid-cols-3 gap-2">
                  <Mini
                    icon={TrendingUp}
                    label="Daily"
                    value={`+${formatMoney(localDaily, cur)}`}
                  />
  ...
  return (
    ...
    >
      <div
        ...
        className="w-full max-w-md rounded-t-3xl sm:rounded-3xl border border-border bg-card p-5 shadow-2xl"
      >
        ...
        <div className="mt-4 rounded-xl border border-warning/30 bg-warning/5 p-3 text-[11px] text-muted-foreground">
          You can only purchase this product once. Daily earnings credit your wallet automatically
          each day and are available for withdrawal.
        </div>
...
function RiskBadge({ level }: { level: string }) {
  ...
  return (
    <span
      className={`rounded-full border px-2 py-0.5 text-[10px] capitalize ${map[level] ?? map.low}`}
    >
      {level} risk
    </span>
  );
