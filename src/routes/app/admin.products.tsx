...
function AdminProducts() {
  ...
  const openCreate = () => {
    setForm(blankForm); setEditing(null); setCreating(true);
  };
  ...
  const save = async () => {
    ...
    if (!isFinite(payload.daily_earning) || payload.daily_earning < 0) return toast.error("Daily earning invalid");
    if (!isFinite(payload.duration_days) || payload.duration_days <= 0) return toast.error("Duration must be > 0");
    if (!isFinite(payload.total_return) || payload.total_return <= 0) return toast.error("Total return must be > 0");
  ...
  return (
    <div className="px-5 pt-2 pb-8">
      <ScreenHeader title="Products" subtitle={`${items.length} plans`} fallbackTo="/app/admin"
        right={<button onClick={openCreate}
      ...
      {!loading && items.length === 0 && (
        <div className="rounded-2xl border border-dashed border-border p-10 text-center text-sm text-muted-foreground">
          No products yet. Click <span className="font-semibold text-foreground">New</span> to add one.
        </div>
      )}

      <div className="space-y-3">
        {items.map((p) => (
          <article key={p.id} className={`rounded-2xl border p-4 ${p.active ? "border-border bg-card" : "border-dashed border-muted bg-card/50 opacity-70"}`}>
            ...
            <div className="mt-3 flex flex-wrap gap-1.5">
              <button onClick={() => openEdit(p)} className="inline-flex items-center gap-1 rounded-md border border-border px-2 py-1 text-[10px] font-semibold hover:bg-background/40">
                <Pencil className="h-3 w-3" /> Edit
              </button>
              <button onClick={() => toggleActive(p)} className="inline-flex items-center gap-1 rounded-md border border-border px-2 py-1 text-[10px] font-semibold hover:bg-background/40">
                <Power className="h-3 w-3" /> {p.active ? "Retire" : "Activate"}
              </button>
              <button onClick={() => remove(p)} className="inline-flex items-center gap-1 rounded-md border border-destructive/40 px-2 py-1 text-[10px] font-semibold text-destructive hover:bg-destructive/10">
      ...
      {creating && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 sm:items-center" onClick={() => !busy && setCreating(false)}>
          <div className="w-full max-w-md rounded-t-3xl border border-border bg-card p-5 sm:rounded-3xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-start justify-between">
              <h3 className="text-base font-semibold">{editing ? "Edit product" : "New product"}</h3>
            ...
            <div className="mt-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <Field label="Daily earning (USD)">
                  <input type="number" value={form.daily_earning} onChange={(e) => setForm({ ...form, daily_earning: e.target.value })}
                    className="w-full rounded-lg border border-border bg-background/30 px-3 py-2 text-sm outline-none focus:border-primary" />
                </Field>
                <Field label="Total return (USD)">
                  <input type="number" value={form.total_return} onChange={(e) => setForm({ ...form, total_return: e.target.value })}
                    className="w-full rounded-lg border border-border bg-background/30 px-3 py-2 text-sm outline-none focus:border-primary" />
                </Field>
              </div>
              <div className="grid grid-cols-2 gap-2">
            ...
            <div className="mt-5 grid grid-cols-2 gap-2">
              <button onClick={() => setCreating(false)} disabled={busy} className="rounded-xl border border-border py-2.5 text-sm font-semibold hover:bg-background/40 disabled:opacity-50">Cancel</button>
              <button onClick={save} disabled={busy} className="rounded-xl bg-primary py-2.5 text-sm font-semibold text-primary-foreground hover:opacity-90 disabled:opacity-50">
