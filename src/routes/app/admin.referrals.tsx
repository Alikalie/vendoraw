...
type App = {
  id: string; user_id: string; status: string; desired_code: string;
  full_name: string; email: string; payout_account: string; created_at: string;
  reject_reason: string | null;
};
type Affiliate = {
  id: string; first_name: string; last_name: string; email: string | null;
  referral_code: string; total_earned: number; currency: string;
};
...
function AdminAffiliates() {
  ...
  const load = async () => {
    ...
    const [a, p] = await Promise.all([
      supabase.from("referral_applications")
        .select("id,user_id,status,desired_code,full_name,email,payout_account,created_at,reject_reason")
    ...
    if (affs.length) {
      ...
      const { data: refs } = await supabase
        .from("profiles").select("referred_by").in("referred_by", ids);
  ...
  const approve = async (id: string) => {
    setBusy(id);
    const { error } = await supabase.rpc("approve_referral_application" as never, { _app_id: id } as never);
    setBusy(null);
  ...
  const reject = async (id: string) => {
    ...
    setBusy(id);
    const { error } = await supabase.rpc("reject_referral_application" as never, { _app_id: id, _reason: reason } as never);
    setBusy(null);
    if (error) return toast.error(error.message);
  ...
  return (
    <div className="px-5 pt-2 pb-8">
      ...
      <p className="mt-1 text-xs text-muted-foreground">Review applications, manage approved affiliates and track commissions.</p>
        ...
        {(["pending", "approved", "rejected", "affiliates"] as const).map((t) => (
          <button
            ...
            className={`whitespace-nowrap rounded-full border px-3 py-1.5 text-xs font-semibold capitalize ${
              tab === t ? "border-primary bg-primary text-primary-foreground" : "border-border text-muted-foreground hover:bg-card"
            }`}
          >
      ...
      <div className="mt-4 space-y-2">
        ...
        {!loading && tab !== "affiliates" && filtered.map((a) => (
          <article key={a.id} className="rounded-2xl border border-border bg-card p-4">
            <header className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <div className="text-[11px] text-muted-foreground">{a.email} · {new Date(a.created_at).toLocaleString()}</div>
              </div>
              <code className="rounded-md bg-muted px-2 py-0.5 text-xs font-bold tracking-wider">{a.desired_code}</code>
            </header>
            <div className="mt-2 grid grid-cols-2 gap-2 text-[11px]">
              <div><span className="text-muted-foreground">Payout:</span> {a.payout_account}</div>
              <div><span className="text-muted-foreground">User ID:</span> <code className="text-[10px]">{a.user_id.slice(0, 8)}…</code></div>
            </div>
            {a.reject_reason && (
              <p className="mt-2 text-[11px] text-destructive">Rejection reason: {a.reject_reason}</p>
            )}
            {a.status === "pending" && (
        ...
        {!loading && tab === "affiliates" && (
          <div className="rounded-2xl border border-border bg-card divide-y divide-border">
            {affiliates.length === 0 && (
              <div className="p-8 text-center text-sm text-muted-foreground">No approved affiliates yet.</div>
            )}
            {affiliates.map((a) => (
              <div key={a.id} className="flex items-center gap-3 px-4 py-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="truncate text-sm font-semibold">{a.first_name} {a.last_name}</div>
                  <div className="truncate text-[11px] text-muted-foreground">{a.email}</div>
                </div>
                <div className="text-right">
                  <code className="rounded bg-muted px-1.5 py-0.5 text-[10px] font-bold tracking-wider">{a.referral_code}</code>
                  <div className="mt-0.5 flex items-center justify-end gap-2 text-[10px] text-muted-foreground">
                    <span className="inline-flex items-center gap-0.5"><Users className="h-3 w-3" /> {signupCounts[a.id] ?? 0}</span>
                    <span className="text-success font-semibold">+{Number(a.total_earned).toFixed(2)} {a.currency}</span>
                  </div>
                </div>
              </div>
            ))}
