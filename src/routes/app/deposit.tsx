  ...
  return (
    <div className="px-5 pt-6 pb-8">
      <div className="flex items-center justify-between">
        <button
          onClick={step === "submitted" ? () => nav({ to: "/app" }) : goBack}
          ...
          {(["amount", "instructions", "proof", "review"] as Step[]).map((s, i) => (
            <div
              ...
              className={`h-1 flex-1 rounded-full ${
                ["amount", "instructions", "proof", "review"].indexOf(step) >= i
                  ? "bg-primary"
                  : "bg-border"
              }`}
            />
      ...
      <p className="mt-1 text-xs text-muted-foreground">
        Available balance:{" "}
        <span className="font-semibold text-foreground">{formatMoney(profile.balance, cur)}</span>
      </p>
      ...
      {step === "amount" && (
        <div className="mt-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button
              onClick={cancel}
              className="rounded-xl border border-border py-3 text-sm font-semibold hover:bg-card"
            >
              Cancel
            </button>
            <button
              ...
              <div className="mt-3 space-y-1.5">
                {accountFields.map((f) => (
                  ...
                  >
                    ...
                    <button
                      onClick={() => copyAccountLine(f.value)}
          ...
          {previewUrl && (
            <div className="rounded-2xl border border-border bg-card p-3">
              ...
              <img
                src={previewUrl}
                alt="proof"
                className="max-h-48 w-full rounded-lg object-contain bg-background/40"
              />
            </div>
          )}
          <div className="grid grid-cols-2 gap-2">
            <button
              onClick={cancel}
              disabled={busy}
              className="rounded-xl border border-border py-3 text-sm font-semibold hover:bg-card disabled:opacity-50"
            >
              Cancel
            </button>
            <button
      ...
      {step === "submitted" && (
        <div className="mt-6 space-y-4">
          <div className="rounded-2xl border border-warning/40 bg-warning/5 p-5 text-center">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Your deposit of{" "}
              <span className="font-semibold text-foreground">{formatMoney(amt, cur)}</span> is
              pending. You'll be notified instantly when it's approved and your wallet is updated
              automatically.
            </p>
          </div>
          <div className="grid grid-cols-2 gap-2">
            <Link
              to="/app"
              className="rounded-xl border border-border py-3 text-center text-sm font-semibold hover:bg-card"
            >
              Home
            </Link>
            {submittedId && (
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-3 px-4 py-3">
      ...
      <span className="text-sm font-semibold text-right truncate max-w-[200px]" title={value}>
        {value}
      </span>
    </div>
  );
