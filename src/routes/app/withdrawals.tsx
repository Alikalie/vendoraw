...
function statusStyle(s: string) {
  ...
  if (s === "pending")
    return { label: "Pending", icon: Clock, cls: "bg-warning/15 text-warning border-warning/30" };
...
function WithdrawalsTab() {
  ...
  const filtered =
    tab === "all"
      ? rows
      : rows.filter((r) => r.status === tab || (tab === "failed" && r.status === "rejected"));
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Track your payout requests and statuses
          </p>
        </div>
        {isAdmin && (
      ...
      <div className="mt-4 grid grid-cols-2 gap-2">
        ...
        <div className="rounded-2xl border border-success/30 bg-success/5 p-3">
          ...
          <div className="mt-0.5 text-sm font-bold text-success">
            {formatMoney(totals.completed, cur)}
          </div>
        </div>
      </div>
      ...
      {!adminView && (
        <div className="mt-4 grid grid-cols-2 gap-2">
          <button
            ...
            className="flex items-center justify-center gap-2 rounded-xl bg-primary py-3 text-sm font-semibold text-primary-foreground hover:opacity-90"
          >
            <Plus className="h-4 w-4" /> New withdrawal
          </button>
          <button
            onClick={() => setShowMethods(true)}
            className="flex items-center justify-center gap-2 rounded-xl border border-border py-3 text-sm font-semibold hover:bg-card"
          >
            Methods
          </button>
        </div>
          ...
          return (
            ...
            >
              ...
              <div className="flex flex-col items-end gap-1">
                ...
                <span
                  className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-medium ${st.cls}`}
                >
