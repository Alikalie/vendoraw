...
function AdminContent() {
  ...
  const save = async () => {
    ...
    setBusy(true);
    const { error } = await supabase.from("site_content" as never).upsert({
      id: draft.id,
      title: draft.title,
      body: draft.body,
  ...
  return (
    <div className="px-5 pt-2 pb-8">
      <ScreenHeader
        ...
        subtitle="Edit user-facing copy"
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              placeholder="new_page_id"
              className="flex-1 rounded-lg border border-border bg-background/30 px-3 py-1.5 text-xs outline-none focus:border-primary"
            />
            <button
              onClick={addPage}
              className="inline-flex items-center gap-1 rounded-lg border border-border px-3 py-1.5 text-xs font-semibold hover:bg-background/40"
            >
