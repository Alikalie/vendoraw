  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 sm:items-center" onClick={onClose}>
      <div className="w-full max-w-md rounded-t-3xl border border-border bg-card p-5 sm:rounded-3xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mt-1 text-xs text-muted-foreground">Available: {formatMoney(profile.balance, cur)}</div>

        {methods.length === 0 ? (
          <div className="mt-5 space-y-3 text-center">
            <p className="text-sm text-muted-foreground">Add a withdrawal method first.</p>
            <button onClick={() => { onClose(); onManageMethods(); }} className="w-full rounded-xl bg-primary py-2.5 text-sm font-semibold text-primary-foreground">
              Add a method
            </button>
          </div>
        ) : (
          <div className="mt-4 space-y-3">
            <label className="block">
              ...
              <select value={methodId} onChange={(e) => setMethodId(e.target.value)}
                ...
                {methods.map((m) => (
                  <option key={m.id} value={m.id}>{m.label}{m.is_default ? " (default)" : ""}</option>
                ))}
              </select>
            </label>
            <label className="block">
